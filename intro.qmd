...
\end{bmatrix}
$$
Vamos ter que na diagonal principal é a variância dos erros e $\forall \, \mathbb{E}(u_{i}, u_{j})$ em que $i \neq j$ temos a covariância dos erros. Sob as hipóteses de homoscedasticidade\footnote{Variância dos erros é constante, isto é, $\mathbb{E}(u_i^2) = \sigma^2, \, \forall i=1, \ldots, n$} e não autocorrelação, vamos ter que:
...
pop_mean = 10  # Média populacional
pop_std = 2  # Desvio padrão populacional
n_simulations = 300  # Número de amostras/simulações
sample_sizes = np.arange(1, n_simulations + 1)  # Tamanhos das amostras
...
# Função para gerar a distribuição das médias das amostras
def plot_distribution(sample_size, num_samples, ax, title):
    np.random.seed(0)
    ...
    # Criar um DataFrame para armazenar as médias das amostras
    df = pd.DataFrame(sample_means, columns=['Mean'])
    
    # Plotar a distribuição das médias das amostras
    ax.hist(df['Mean'], bins=30, density=True, alpha=0.7, color='blue')
    ax.set_title(title)
    ax.set_xlabel('Média das Amostras')
    ax.set_ylabel('Densidade')
    ax.grid(True)

# Definir tamanhos de amostra diferentes
sample_sizes = [10, 100, 1000]
num_samples = 1000

# Configurar a figura com 3 subplots
fig, axs = plt.subplots(1, 3, figsize=(18, 6))

# Gerar e plotar os gráficos
for ax, size in zip(axs, sample_sizes):
    plot_distribution(size, num_samples, ax, f'Tamanho da Amostra = {size}')
